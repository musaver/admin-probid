...
interface Item {
  ...
  recordedNumber: string | null; title: string | null;
}
interface Claim {
...
export default function BidderVerificationPage() {
  ...
  const verify = async (id: string) => {
    setBusyId(id);
    try {
      const res = await fetch(`/api/bidder-claims/${id}/verify`, { method: 'POST' });
      if (!res.ok) { const e = await res.json().catch(() => ({})); alert(e.error || 'Failed to verify'); }
      else { const r = await res.json(); alert(`Verified — ${r.linked} of ${r.total} propert${r.total === 1 ? 'y' : 'ies'} linked to the bidder.`); await load(); }
              ...
              <CardContent className="p-4">
                <div className="flex items-start justify-between mb-3">
                  ...
                  <div className="text-right">
                    ...
                    <div className="text-xs text-muted-foreground mt-1">{matchedCount}/{c.items.length} match</div>
                  </div>
                </div>
                  ...
                  {c.items.map((it) => (
                    <div key={it.id} className="flex items-center gap-2 px-3 py-2">
                      ...
                      <span className="ml-auto text-xs text-muted-foreground">
                        {matchText[it.match]}{it.recordedNumber ? ` (property #${it.recordedNumber})` : ''}
                      </span>
                    </div>
                  ))}
                </div>
                ...
                {c.status === 'pending' && (
                  <div className="flex gap-2 justify-end mt-3">
                    <Button size="sm" disabled={busyId === c.id} onClick={() => verify(c.id)}>
                      {busyId === c.id ? <Loader2 className="h-3 w-3 animate-spin" /> : <><Check className="h-3 w-3 mr-1" /> Verify ({matchedCount})</>}
                    </Button>
